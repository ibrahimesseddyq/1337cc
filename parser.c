...
void parser_reorder_expression(struct node** node_out)
{
    ...
    if (((is_array_node(node->exp.left) || is_node_assignement(node->exp.right))) ||
        (node_is_expression(node->exp.left, "()") ) && node_is_expression(node->exp.right, ","))
    {
...
void parse_parse_for_array(struct history* history)
{
...
struct token* parser_build_random_type_name()
{
    ...
    char *sval = malloc(sizeof(tmp_name)+1);
...
void parser_datatype_init_type_and_size_for_primitive(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out)
{
    ...
    {
        compiler_error(current_process, "Your not allowed a secondary datatype here for the given datatype");
    }
...
void parser_datatype_init_type_and_size(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out, int pointer_depth, int expected_type)
{
    ...
    {
        compiler_error(current_process, "You procided an invalid secondary datatype");
    }     
...
{
    parse_expressionable(history);
    struct node* resutl_node = node_pop();
    node_push(resutl_node);
}
...
{
    struct datatype_struct_node_fix_private* private = fixup__private(fixup);
...
void make_variable_node(struct datatype* dtype, struct token* name_token, struct node* value_node)
{
    ...
    if (var_node->type == DATA_TYPE_STRUCT && !var_node->var.type.struct_node)
    {
...
void parse_body_single_statement(size_t* variable_size, struct vector* body_vec, struct history* history)
{
    ...
    // change the variable_size variable by the size of the node
...
void parse_body_multiple_statements(size_t* variable_size, struct vector* body_vec, struct history* history)
{
    ...
    while (!token_next_is_symbol('}'))
    {
        parse_statement(history_down(history, history->flags));
        stat_node = node_pop();
        if (stat_node->type == NODE_TYPE_VARIABLE)
        {
            if (!largest_possible_var_node || (largest_possible_var_node->var.type.size <= stat_node->var.type.size))
            {
                largest_possible_var_node = stat_node; 
            }

            if (variable_node_is_primitive(stat_node))
            {
                if (!largest_align_eligible_var_node || (largest_align_eligible_var_node->var.type.size <= stat_node->var.type.size))
                {
                    largest_align_eligible_var_node = stat_node; 
                }
            }
        }
        vector_push(body_vec, &stat_node);

        parser_append_size_for_node(history, variable_size, variable_node_or_list(stat_node));
    }
...
void parse_struct_no_new_scope(struct datatype* dtype, bool is_forward_declaration)
{
    ...
    if (token_is_identifier(token_peek_next()))
    {
        ...
        if (dtype->flags & DATATYPE_FLAG_IS_STRUCT_UNION_NO_NAME)
        {
            ...
            dtype->flags &= ~DATATYPE_FLAG_IS_STRUCT_UNION_NO_NAME;
            struct_node->_struct.name = var_name;
        }
...
void parse_variable_function_or_struct_union(struct history* history)
{
    ...
    if (datatype_is_struct_or_union(&dtype) && token_next_is_symbol('{'))
    {
        ...
        symbolresolver_build_for_node(current_process, su_node);
...
void parse_case(struct history* history)
{
    ...
    {
        compiler_error(current_process, "we only support numbers in our subset") ;
    }
...
int parse(struct compile_process* process)
{
    ...
    assert(fixup_resolve(parser_fixup_sys));
    return PARSE_ALL_OK;
}