...
{
    return node->type == NODE_TYPE_EXPRESSION || node->type == NODE_TYPE_EXPRESSION_PARENTHESIS 
...
{
    node_create(&(struct node){.type=NODE_TYPE_STATEMENT_WHILE, .stmt.do_while_stmt.exp_node=exp_node, .stmt.do_while_stmt.body_node=body_node});
...
{
    struct symbol* sym = symbolresolver_get_symbol(current_process, name);
...
{
    return node_is_expression_or_parentheses(node) || node->type == NODE_TYPE_IDENTIFIER ||
        node->type == NODE_TYPE_NUMBER || node->type == NODE_TYPE_UNARY ||
        node->type == NODE_TYPE_TERNARY || node->type == NODE_TYPE_STRING;
}